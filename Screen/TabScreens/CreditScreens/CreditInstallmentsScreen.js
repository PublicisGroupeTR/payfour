...
const CreditInstallmentsScreen = ({navigation, route}) => {
  ...
  "firstInstallmentDate": "2024-12-25"
      ...
      apiPost('loans/calculatepaybackplan', sendData, onCalculatePaybackPlan);
...
const onCalculatePaybackPlan = (response) =>{
  ...
  const handleSubmitPress = () =>{
    ...
    AsyncStorage.getItem('token').then(value =>{
      ...
      apiPost('payments/approvewithloan', sendData, onApproveWithLoan);
      ...
      <ScrollView style={{
        ...
        <View style={{borderRadius:16,paddingBottom:100}}>
          ...
          <View style={{
            ...
            {renderInstallments()}
            <TouchableOpacity
            style={[regstyles.buttonStyle, {display:installmentArr.length >0? 'flex' : 'none', padding:0, backgroundColor: '#004F97', flex:1, marginBottom:12,marginTop:24,}]}              
            activeOpacity={0.5}
            onPress={handleSubmitPress}>
            <Text style={regstyles.buttonTextStyle}>Alışveriş Kredisi ile Öde</Text>
          </TouchableOpacity>
          <TouchableOpacity
